    ...
    <?php if (get_field('show_header_image')) : ?>
        ...
        <div id="ups-block" class="ups-block">
            ...
            while ($the_query->have_posts()) : $the_query->the_post(); ?>
                ...
                if ('LosAngeles' == get_field('fuso_orario', get_the_ID()) || 'OC' == get_field('fuso_orario', get_the_ID())) {
                    $today_timepstamp = $today->sub(new DateInterval("PT6H"))->format('YmdH');
                }

                if ($timestamp >= $today_timepstamp && $count_ev <= $events_num) : $count_ev++ ?>
                    <div class="block-event-single <?php echo ('LosAngeles' == get_field('fuso_orario', get_the_ID()) || 'OC' == get_field('fuso_orario', get_the_ID())) ? 'odd' : 'even'; ?>">
                        <div class="block-event-field block-event-img" style="background-position:center; background-image:url('<?php echo get_the_post_thumbnail_url(get_the_ID(), 'medium'); ?>'); ">
                        </div>
                        ...
                        <div class="block-event-field block-event-btn-mobile">
                            ...
                            <div>
                                ...
                                <?php if (get_field('event_external_link', get_the_ID())) : ?>
                                    <a class="btn" target="_blank" href="<?php echo get_field('event_external_link', get_the_ID()); ?>">
                                        TIX
                                    </a>
                                <?php endif; ?>
                            </div>
