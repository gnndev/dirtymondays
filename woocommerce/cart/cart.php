			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						<td class="dm-cart-product-thumbnail">
						<?php
						...
						if ( ! $product_permalink ) {
							...
							echo '<a href="'.esc_url( $product_permalink ).'"><strong>'.$name.'</strong></a>' ;
						}
						...
						echo '<p class="product-cart-variations">';
						...
						if(!empty($cart_item['variation'])) {
							foreach ($cart_item['variation'] as $key => $variation) {
								echo '<span style="text-transform:uppercase">'. str_replace( ['_', 'pa', 'attribute'], '', $key,) .': '. $variation .'</span> / ';
							}
						}
						echo '<span>QTY: '. $cart_item['quantity'] . '</span></p>';
